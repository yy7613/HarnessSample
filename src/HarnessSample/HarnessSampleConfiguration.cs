    ...
    WebSearchToolConfiguration WebSearch)
{
    public static HarnessSampleConfiguration Load()
    {
        ...
        [
            ...
            "この HarnessSample における BackgroundAgents の役割",
            "この HarnessSample における TodoProvider / AgentModeProvider / BackgroundAgentsProvider / FileMemoryProvider の役割整理",
        ...
        - Step02: BackgroundAgentsProvider を使って、親エージェントから複数のサブエージェントへ委譲する
        ...
        - AIContextProviders として TodoProvider / AgentModeProvider / BackgroundAgentsProvider / FileMemoryProvider を使う
        - オプションで Web検索 Tool を使える
        - plan モードでは TODO と実行方針を作る
        - execute モードでは BackgroundAgentsProvider を使ってサブエージェントへ依頼し、必要なら Web検索 Tool も使って結果を統合する
